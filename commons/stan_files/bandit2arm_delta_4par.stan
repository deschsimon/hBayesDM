...
transformed parameters {
  ...
  d    = Phi_approx(mu_pr[4] + sigma[4] * d_pr);
}
model {
  // Hyperparameters
  ...
  d_pr      ~ normal(d_pr_m,    d_pr_sd);

  // subject loop and trial loop
  for (i in 1:N) {
    vector[2] ev; // expected value
    vector[2] tmp;// temporary vector for decay update 
