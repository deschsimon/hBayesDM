#' @templateVar MODEL_FUNCTION bandit2arm_delta_6par
...
#' @templateVar MODEL_NAME Rescorla-Wagner (Delta) Model (6 parameters)
#' @templateVar MODEL_CODE delta_ext
#' @templateVar MODEL_CITE 
#' @templateVar MODEL_TYPE Hierarchical
#' @templateVar DATA_COLUMNS "subjID", "choice", "outcome"
#' @templateVar PARAMETERS \code{Arew} (reward learning rate), \code{Apun} (punishment learning rate), \code{tau} (inverse temperature), \code{R} (reward sensitivity), \code{P} (punishment sensitivity), \code{d} (decay rate)
...
bandit2arm_delta_6par <- hBayesDM_model(
  task_name       = "bandit2arm",
  model_name      = "delta_6par",
  model_type      = "",
